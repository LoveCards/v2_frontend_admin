...
<template>
  <NuxtLayout name="root">
    ...
    <v-row class="pt-2">
      <v-col cols="6">
        <h1 class="text-primary font-weight-bold">角色管理</h1>
      </v-col>
    </v-row>
        ...
        <v-card class="pa-4">
          ...
          <v-row>
            <v-col cols="4" md="8" class="d-flex align-center">
              <v-btn color="accent" variant="flat" @click="CreateDialog_state = true">创建角色</v-btn>
            </v-col>
              ...
              <v-card variant="outlined" color="#E0E0E0">
                <v-data-table :style="{ 'white-space': 'nowrap' }" :headers="TableHeaders" :items="tableItems"
                  ...
                  <template v-slot:[`item.operate`]="{ item }">
                    ...
                    <v-btn icon="mdi-shield-key" elevation="0" size="small" variant="text"
                      @click="openAssignDialog(item)" title="分配权限"></v-btn>
  ...
  <CreateRoleDialog v-model:thisDialogState="CreateDialog_state" :getTableData="getTableData"></CreateRoleDialog>
  <EditRoleDialog v-model:thisDialogState="EditDialog_state" v-model:RoleData="EditDialog_data"
    :getTableData="getTableData"></EditRoleDialog>
  <AssignPermissionsDialog v-model:thisDialogState="AssignDialog_state" v-model:RoleId="AssignDialog_roleId"
    :getTableData="getTableData"></AssignPermissionsDialog>
...
import CreateRoleDialog from "@/components/apps/roles/CreateRoleDialog.vue";
...
// 分配权限
const AssignDialog_state = ref(false);
const AssignDialog_roleId = ref<number>(0);
const openAssignDialog = (item: any) => {
  AssignDialog_roleId.value = item.id;
  AssignDialog_state.value = true;
};
